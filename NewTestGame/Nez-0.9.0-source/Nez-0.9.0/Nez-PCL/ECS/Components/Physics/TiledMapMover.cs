...
namespace Nez.Tiled
{
	...
	#endif
	{
        ...
        /// <summary>
        /// the list TiledTileLayers used for collision checks (Changed from a single variable to a list to allow for multiple collision layers)
        /// </summary>
        public readonly List<TiledTileLayer> collisionLayers = new List<TiledTileLayer>();
		...
		void populateCollidingTiles( Rectangle bounds, Edge direction )
		{
            ...
            foreach (TiledTileLayer currentLayer in collisionLayers)
            {
                for (var primary = firstPrimary; primary != lastPrimary + primaryIncr; primary += primaryIncr)
                {
                    for (var secondary = firstSecondary; secondary != lastSecondary + secondaryIncr; secondary += secondaryIncr)
                    {
                        ...
                        _collidingTiles.Add(currentLayer.getTile(col, row));
